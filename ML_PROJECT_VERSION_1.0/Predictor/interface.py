...
from PyQt5.QtCore import Qt,QTimer,QDate,QRect
...
class mainScreen(QWidget):
    def __init__(self,userIndex, *args, **kwargs):
        ...
        self.databasePath ="{}Databases/user{}.db".format(self.dir_path,hex(userIndex)[2:])
    ...
    def efficiencyPredictionButtonFunction(self):#verim tahmin sayfasına yönlendirir.
        popWindow = PredictionForm(self.data_dict)
        popWindow.show()
    ...
    def saveToDataBase(self):#Seçili tarihte girili bilgileri veritabanınına kayıt eder.
        ...
        if not bool(len(errorColumns)):
            ...
            self.textCheckout.setText("Bilgiler kaydedilmiştir ")
            ...
            self.textCheckout.setText("Geçersiz saat aralığı girişi.Hata kaynağı sütunları : {} ".format(columnsText))
            self.textCheckout.setStyleSheet("""
            font-family:Courier;
            font-size:14px;
            font-weight:650;
    ...
    def changeHorizontalHeader(self,index):
        ...
        if index == -1:
            ...
            self.tableWidget.setColumnCount(index+1)

        try:
            last_range = self.tableWidget.horizontalHeaderItem(index).text()
            new_range, ok = QInputDialog.getText(self,
                                            'Yeni saat Aralığı {}'.format(index),
                                            'Saat Aralığı:',
                                            QLineEdit.Normal,
                                            last_range)
            if ok:
                self.tableWidget.horizontalHeaderItem(index).setText(new_range)
        except :
            new_range, ok = QInputDialog.getText(self,
                                            'Yeni saat Aralığı {}'.format(index),
                                            'Saat Aralığı:',
            ...
            item = QTableWidgetItem(new_range)
            self.tableWidget.setHorizontalHeaderItem(index,item)

        try:
            time = [hour.split(":") for hour in new_range.split("-")]
            self.tableWidget.setColumnWidth(index,150)
